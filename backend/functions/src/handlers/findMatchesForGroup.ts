...
export const findMatchesForGroup = functions.https.onCall(
  ...
  if (!groupId || !weights) {
    throw new functions.https.HttpsError("invalid-argument", "groupId and weights are required");
  }
  ...
  if (!currentStats) {
    throw new functions.https.HttpsError("not-found", "Group stats not found");
  }
  ...
  for (const [otherId, otherStats] of blobMap.entries()) {
    ...
    // Hard filters
    if (weights.commute === 5 && otherStats[FIELD_INDEX.maxCommute] > currentStats[FIELD_INDEX.maxCommute]) {
      continue;
    }
    if (weights.budget === 5 && otherStats[FIELD_INDEX.maxBudget] > currentStats[FIELD_INDEX.maxBudget]) {
      continue;
    }
    if (weights.profilePicture === 5 && otherStats[FIELD_INDEX.profilePictureRatio] < 1.0) {
      continue;
    }
    if (weights.age === 5 && otherStats[FIELD_INDEX.maxAge] > currentStats[FIELD_INDEX.maxAge]) {
      continue;
    }
    if (weights.age === 5 && otherStats[FIELD_INDEX.minAge] < currentStats[FIELD_INDEX.minAge]) {
      continue;
    }
    if (weights.bedtime === 5 && otherStats[FIELD_INDEX.maxBedtime] > currentStats[FIELD_INDEX.maxBedtime]) {
      continue;
    }
    if (weights.alcohol === 5 && otherStats[FIELD_INDEX.maxAlcohol] > currentStats[FIELD_INDEX.maxAlcohol]) {
      continue;
    }
    if (weights.alcohol === 5 && otherStats[FIELD_INDEX.minAlcohol] < currentStats[FIELD_INDEX.minAlcohol]) {
      continue;
    }
    ...
    function apply(weight: number, value: number) {
        if (weight <= 1) return; // slider = 1 means ignore
        score += value * weight;
        totalWeight += weight;
    }
