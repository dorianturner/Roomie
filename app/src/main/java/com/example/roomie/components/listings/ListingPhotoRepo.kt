...
suspend fun fetchListingPhotos(listingId: String): List<PhotoItem> {
    val doc = FirebaseFirestore.getInstance().collection("listings").document(listingId).get().await()
...
suspend fun deleteListingPhoto(listingId: String, path: String): Boolean {
    ...
    val listingRef = FirebaseFirestore.getInstance().collection("listings").document(listingId)
