...
fun ProfileEditorScreen(
    ...
    val phoneNumberField =
        remember { mutableStateOf(ProfileTextField("Phone Number", "", KeyboardType.Phone, false)) }
    ...
    // Student fields
    val ageField =
        remember { mutableStateOf(ProfileTextField("Your Age", "", KeyboardType.Number)) }
    val universityField = remember { mutableStateOf(ProfileTextField("Your University", "")) }
    val preferencesField =
        remember { mutableStateOf(ProfileTextField("Basic Preferences", "", required = false)) }
    val groupSizeMinField =
        remember { mutableStateOf(ProfileTextField("Min", "", KeyboardType.Number)) }
    val groupSizeMaxField =
        remember { mutableStateOf(ProfileTextField("Max", "", KeyboardType.Number)) }
    val maxCommuteField =
        remember { mutableStateOf(ProfileTextField("Max Commute (mins)", "", KeyboardType.Number)) }
    val maxBudgetField = remember {
        mutableStateOf(
            ProfileTextField(
                "Max Budget (£ / week)",
                "",
    ...
    val addictedField =
        remember { mutableStateOf(ProfileTextField("My ideal night is...", "", required = false)) }
    val idealField = remember {
        mutableStateOf(
            ProfileTextField(
    ...
    val passionateField =
        remember { mutableStateOf(ProfileTextField("I am passionate about", "", required = false)) }
                ...
                .addOnSuccessListener { doc ->
                    if (doc.exists()) {
                        ...
                        if (isLandlord) {
                            ...
                            universityField.value.value =
                                doc.getString("studentUniversity").orEmpty()
                            preferencesField.value.value =
                                doc.getString("studentBasicPreferences").orEmpty()
                            val group = doc.get("studentDesiredGroupSize") as? List<*>
                            groupSizeMinField.value.value =
                                (group?.getOrNull(0) as? Long)?.toString().orEmpty()
                            groupSizeMaxField.value.value =
                                (group?.getOrNull(1) as? Long)?.toString().orEmpty()
                            maxCommuteField.value.value =
                                doc.getLong("studentMaxCommute")?.toString().orEmpty()
                            maxBudgetField.value.value =
                                doc.getLong("studentMaxBudget")?.toString().orEmpty()
                            // load lifestyle fields
                            smokingStatusState.value =
                                doc.getString("studentSmokingStatus") ?: "Neither"
                            ...
                            addictedField.value.value = doc.getString("studentAddicted")
                                .orEmpty() // Label: "My ideal night is..."
                            idealField.value.value = doc.getString("studentIdeal")
                                .orEmpty()       // Label: "I am completely addicted to"
                            passionateField.value.value =
                                doc.getString("studentPassionate").orEmpty()
                            groupId = doc.getString("groupId")
                            Log.d(
                                "ProfileEditorScreen",
                                "Loaded profile for user $uid, groupId = $groupId, isLandlord = $isLandlord"
                            )
                        }
                    }
                }
            ...
            if (isLandlord) {
                ...
                ProfilePhotosEdit(
                    ...
                    onPhotosChanged = { updated ->
                        photos =
                            updated // 'photos' state is updated, used when saving if user becomes landlord
                    }
                )

                Spacer(modifier = Modifier.height(Spacing.extraLong))
            ...
            Button(
                onClick = {
                    ...
                    if (currentUser != null) {
                        ...
                        var isMinProfileSet =
                            true // Flag to check if minimum required fields for a profile type are set
                        ...
                        if (isLandlord) {
                            ...
                            data["studentMaxCommute"] =
                                commute ?: 0 // Default to 0 if not specified
                            ...
                            if (smokingStatusState.value.isBlank() || bedtimeState.intValue !in 1..5 || alcoholState.intValue !in 1..5) {
                                Toast.makeText(
                                    context,
                                    "Please fill smoking, bedtime and drinking preferences.",
                                    Toast.LENGTH_SHORT
                                ).show()
                                return@Button
                            }
                            ...
                            data["studentAddicted"] =
                                addictedField.value.value // "My ideal night is..."
                            data["studentIdeal"] =
                                idealField.value.value       // "I am completely addicted to"
                            ...
                            if (groupId == null) {
                                ...
                                Log.d(
                                    "ProfileEditorScreen",
                                    "Creating group for user ${currentUser.uid}"
                                )
                                data["groupId"] =
                                    currentUser.uid // Assign user's UID as their new group ID
                                val members: List<StudentProfile> = listOf(currentStudent)
                                val stats =
                                    generateGroupStats(members) // Generate stats for the new group
                                ...
                                batch.set(
                                    db.collection("groups").document(currentUser.uid),
                                    groupProfile.toMap()
                                )
                            } else {
                                ...
                                groupRef.get().addOnSuccessListener { groupDoc ->
                                    if (groupDoc.exists()) {
                                        ...
                                        val status: Int =
                                            when (statusAny) { // Handle potential number types for status
                                                is Long -> statusAny.toInt()
                                                is Double -> statusAny.toInt()
                                                is Int -> statusAny
                                                else -> 0 // Default status if not found or wrong type
                                            }
                                        val size = (statsMap?.get("size") as? Number)?.toLong()
                                            ?: 0L // Handle potential number types for size

                                        if (size <= 1) { // Condition to update group if only one person
                                            Log.d(
                                                "ProfileEditorScreen",
                                                "Group $groupId has size $size, updating stats based on current user."
                                            )
                                            // Update group: members list contains only the current student, stats are regenerated
                                            val members: List<StudentProfile> =
                                                listOf(currentStudent)
                                            val newStats =
                                                generateGroupStats(members).copy(status = status) // Preserve existing status
                                            ...
                                            val updatedGroupProfile = GroupProfile(
                                                ...
                                                stats = newStats,
                                                profilePicture = groupData.get("profilePicture") as? String
                                                    ?: "", // Preserve existing
                                                bio = groupData.get("bio") as? String
                                                    ?: ""                         // Preserve existing
                                            )
                                            ...
                                            FunctionsProvider.instance
                                                ...
                                                .addOnSuccessListener { result ->
                                                    Log.d(
                                                        "SaveProfile",
                                                        "Updated group upsert success: ${result.data} for group $groupId"
                                                    )
                                                }
                                                .addOnFailureListener { e ->
                                                    Log.e(
                                                        "SaveProfile",
                                                        "Updated group upsert failure for $groupId",
                                                        e
                                                    )
                                                }
                                            ...
                                            groupRef.set(updatedGroupProfile.toMap()) // Direct set as per original logic structure
                                                .addOnFailureListener { e ->
                                                    Log.e(
                                                        "ProfileEditorScreen",
                                                        "Failed to update group $groupId in Firestore directly.",
                                                        e
                                                    )
                                                }
                                            ...
                                            Log.d(
                                                "ProfileEditorScreen",
                                                "Group $groupId has size $size (>10), not updating stats from profile edit."
                                            )
                                        }
                                    } else {
                                        Log.w(
                                            "ProfileEditorScreen",
                                            "Group $groupId not found, cannot update stats."
                                        )
                                    }
                                }.addOnFailureListener { e ->
                                    Log.e(
                                        "ProfileEditorScreen",
                                        "Failed to get group $groupId for stats update.",
                                        e
                                    )
                                }
                            }
